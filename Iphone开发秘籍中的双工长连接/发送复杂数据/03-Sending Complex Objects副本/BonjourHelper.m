...
@interface BonjourHelper(){


}
@property(strong)NSString *curServiceName;
@end
...
- (void)netServiceDidResolveAddress:(NSNetService *)netService
{
	...
	if (addresses && addresses.count)
	{
		for (int i = 0; i < addresses.count; i++)
		{
            ...
            if([servername isEqualToString:self.curServiceName])
            {
			...
			self.outConnection = [[[TCPServerConnection alloc] initWithRemoteAddress:address] autorelease];
...
+ (void) sendData: (NSData *) data
{
    ...
    BOOL success =NO;
    if(sharedInstance.outConnection){
       success = [sharedInstance.outConnection sendData:data];
    }
    else if(sharedInstance.inConnection){
      success =  [sharedInstance.inConnection sendData:data];
    }
//    	BOOL success = [sharedInstance.inConnection sendData:data];
    if (success) {
...
//服务端客户端成功建立连接，服务端和客户端都会调用
- (void) connectionDidOpen: (TCPConnection *) connection
{
    // Fully opened connection
    printf("Connection did open: %s\n", (connection == self.inConnection) ? "incoming" : "outgoing");
    if (connection == self.inConnection)
    {
        inConnected = YES;
    }
    if (connection == self.outConnection){
        outConnected = YES;
    }
    [self updateStatus];
}
//服务端与客户端断开连接，服务端和客户端都会调用
...
//客户端发过来连接请求，服务端同意了，反馈连接成功，只针对服务端
